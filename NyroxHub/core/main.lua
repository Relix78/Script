...
local Games   = Import("core/gamecheck.lua") or warn("[Nyrox] GameCheck konnte nicht geladen werden!")
local DevMode = Import("core/devmode.lua")
local assets  = Import("assets.lua") or error("[Nyrox] UI Library fehlt!")

env.NyroxToggleStates = env.NyroxToggleStates or {}
function Core.Init()
    if not Games or not assets or type(Games) ~= "table" then
        warn("[Nyrox] Initialisierung abgebrochen: Abhängigkeiten fehlen.")
        return
    end
